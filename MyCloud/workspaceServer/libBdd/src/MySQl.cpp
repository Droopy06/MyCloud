...
const string MySQL::getUser(const string & adresseMail, const string & password)	{
	sql::PreparedStatement *pstmt;
	pstmt = con->prepareStatement("SELECT idUser FROM user WHERE adresseMail=? and mdp=?");
	pstmt->setString(1, adresseMail);
